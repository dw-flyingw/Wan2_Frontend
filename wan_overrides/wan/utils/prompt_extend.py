...
DEFAULT_SYS_PROMPTS = {
    ...
    "animate-14B": {
        "zh": ANIMATE_ZH_SYS_PROMPT,
        "en": ANIMATE_EN_SYS_PROMPT,
    },
}
...
class OpenAIPromptExpander(PromptExpander):
    """
    Prompt expander that uses OpenAI-compatible API endpoints.
    Works with any OpenAI-compatible server (vLLM, TGI, etc.)
    """
    ...
    def __init__(self,
        ...
        '''
        Args:
            model_name: The model name to use in API calls. If None, uses a default.
        ...
        if base_url is None:
            raise ValueError("base_url is required for OpenAIPromptExpander")
        ...
        super().__init__(model_name, task, is_vl, **kwargs)
        logging.info(f"OpenAIPromptExpander using model: {self.model_name}")

        if is_vl:
            logging.warning("OpenAIPromptExpander does not yet support vision-language tasks")

    def _detect_model_name(self):
        """Query the /v1/models endpoint to get the available model name."""
        try:
            ...
            logging.warning(f"Failed to auto-detect model name: {e}")
        return "default"
    ...
    def extend_with_img(self,
                        ...
                        *args,
                        **kwargs):
        # For now, fall back to text-only extension
        logging.warning("Vision-language prompt extension not supported with OpenAI expander, using text-only")
        return self.extend(prompt, system_prompt, seed, *args, **kwargs)
